<template>
	<div class="select-box">
		...
		<div
			:style="[overflow ? { overflow: 'auto' } : { overflow: 'hidden' }]"
	...
	methods: {
		...
		prevItemFocus(event) {
			...
			if (this.itemFocusIndex == 0) {
				this.itemFocusIndex = this.dropdownData.length - 1;
				return;
			}
			this.itemFocusIndex--;
		},
...
.select-box .select-items-bottom {
	top: auto;
...
.select-box .select-items .select-item {
	outline: none;
	height: 33%;
	cursor: pointer;
	user-select: none;
...
.select-box .select-items .select-item:hover {
	background-color: #e9ebee;
	color: #2CA01C;
}

.select-box .select-items .select-item:focus {
	background-color: #e9ebee;
	color: #000;
}
